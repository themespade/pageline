...
/************************************** THEME-OPTIONS *******************************************************/
  ...
  for( $i = 1; $i <= 4; $i++ ) {
    ...
    $wp_customize->add_control( 'pageline_slide'.$i, array(
      ...
      $wp_customize->add_setting( 'pageline_slide_link'.$i, array(
          'default'               => '',
          'capability'            => 'edit_theme_options',
          'sanitize_callback'     => 'esc_url_raw'
      ));

      $wp_customize->add_control( 'pageline_slide_link'.$i, array(
          'priority'               => $i+10,
          'label'                  => sprintf( __( 'Overwrite Link for Slide  %d', 'pageline' ), $i ),
          'section'                => 'pageline_header_slider',
          'settings'               => 'pageline_slide_link'.$i,
      ));
  }

    $wp_customize->add_setting( 'pageline_slider_button_activation', array(
        'default'               => '',
        'capability'            => 'edit_theme_options',
        'sanitize_callback'     => 'pageline_sanitize_checkbox'
    ) );

    $wp_customize->add_control( 'pageline_slider_button_activation', array(
        'label'           => esc_html__( 'Enable Button' , 'pageline' ),
        'section'         => 'pageline_header_slider',
        'type'            => 'checkbox',
        'priority'        => 30
    ) );

    $wp_customize->add_setting( 'pageline_slider_button_text', array(
        'default'               => 'Read More',
        'capability'            => 'edit_theme_options',
        'sanitize_callback'     => 'sanitize_text_field'
    ));

    $wp_customize->add_control( 'pageline_slider_button_text', array(
        'priority'               => 35,
        'label'                  => esc_html__('Button Text', 'pageline'),
        'section'                => 'pageline_header_slider',
        'settings'               => 'pageline_slider_button_text',
    ));
